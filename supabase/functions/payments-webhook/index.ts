...
async function handleTransactionCompleted(data: any, env: PaddleEnv) {
  // Only count paid invoices, not $0 trial start transactions
  const total = parseFloat(data.details?.totals?.total ?? '0');
  ...
  const priceExternal = item?.price?.importMeta?.externalId ?? null;
  const currency = data.currencyCode ?? 'USD';
  // total is in lowest denomination
  const netAmount = total / 100;

  await getSupabase().rpc('record_affiliate_commission', {
    p_user_id: userId,
    p_net_amount: netAmount,
    p_currency: currency,
    p_plan_type: priceExternal,
  });
}
